...
async function updateFromRawg(){if(importing){cancelImport=true;setMessage('Arrêt demandé après la page en cours.');return}if(!currentUser){setMessage('Connecte-toi avant d’importer des jeux.',true);return}importing=true;cancelImport=false;const button=$('refreshButton');button.innerHTML='■ Arrêter l’import';$('importProgress').hidden=false;let page=Math.max(1,Number(localStorage.getItem(RAWG_PAGE_KEY))||1);let requests=0;let imported=0;try{while(catalog.length<TARGET_CATALOG_SIZE&&requests<MAX_REQUESTS_PER_IMPORT&&!cancelImport){const response=await fetch(`/api/rawg?page=${page}&page_size=40`);const data=await response.json();if(!response.ok)throw new Error(data.error||'API RAWG indisponible');const games=(data.results||[]).map(normalizeGame);games.forEach(game=>{const existing=catalogById.get(game.id);catalogById.set(game.id,existing?{...existing,...game}:game)});catalog=[...catalogById.values()];await saveGames(games);page+=1;requests+=1;imported+=games.length;localStorage.setItem(RAWG_PAGE_KEY,String(page));const progress=Math.min(100,catalog.length/TARGET_CATALOG_SIZE*100);$('importProgressBar').style.width=progress+'%';setMessage(`Import ${catalog.length.toLocaleString('fr-CA')} / ${TARGET_CATALOG_SIZE.toLocaleString('fr-CA')} jeux — page RAWG ${page-1}`);if(!data.next)break;if(requests%5===0){populateFilters();render();await new Promise(resolve=>setTimeout(resolve,150))}}populateFilters();currentPage=1;render();$('apiStatus').textContent='✓ Clé RAWG active dans Netlify.';$('apiStatus').style.color='var(--green)';setMessage(cancelImport?`Import interrompu et sauvegardé à ${catalog.length.toLocaleString('fr-CA')} jeux. Reprends quand tu veux.`:`${imported.toLocaleString('fr-CA')} jeux traités. Catalogue : ${catalog.length.toLocaleString('fr-CA')} jeux.`)}catch(error){$('apiStatus').textContent='✕ Vérifie RAWG_API_KEY dans Netlify et le dernier déploiement.';$('apiStatus').style.color='var(--pink)';setMessage(error.message,true)}finally{importing=false;cancelImport=false;button.innerHTML='<span>↻</span> Importer jusqu’à 10 000';$('importProgress').hidden=catalog.length>=TARGET_CATALOG_SIZE}}
...
async function initialize(){await initializeCatalog();populateFilters();render();onAuthStateChanged(auth,async user=>{const epoch=++authEpoch;currentUser=user;inventoryReady=false;played=new Set();seen=new Set();pendingPlayed.clear();reviewSaving=false;currentPage=1;cancelImport=true;$('showSeen').checked=false;$('reviewMessage').textContent='';if($('gameModal').open)$('gameModal').close();render();$('signOutButton').hidden=!user;$('signInButton').hidden=Boolean(user);$('signUpButton').hidden=Boolean(user);$('authEmail').disabled=Boolean(user);$('authPassword').disabled=Boolean(user);$('syncStatus').className=`top-status ${user?'is-cloud':'is-offline'}`;$('syncStatus').innerHTML=`<span class="status-dot"></span> ${user?'FIREBASE SYNC ACTIVE':'CONNEXION REQUISE'}`;if(user){$('authEmail').value=user.email||'';$('authPassword').value='';authMessage(`Connexion : ${user.email}`);try{await loadCloudPlayedGames(user,epoch);if(epoch!==authEpoch)return;authMessage(`✓ Inventaire privé synchronisé : ${played.size} jeu${played.size!==1?'x':''}`);if(catalog.length<TARGET_CATALOG_SIZE&&!sessionStorage.getItem('gamevault-auto-import')){sessionStorage.setItem('gamevault-auto-import','started');updateFromRawg()}}catch{if(epoch!==authEpoch)return;authMessage('Connectée, mais Firestore refuse l’accès. Vérifie les règles.',true)}}else{if($('gameModal').open)$('gameModal').close();authMessage('Non connecté — aucun inventaire n’est affiché.');render()}})}
